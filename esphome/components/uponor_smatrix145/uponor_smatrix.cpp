...
void UponorSmatrixComponent::loop() {
  ...
  //if (!this->tx_queue_.empty()) {  
    //ESP_LOGI(TAG, "tx_queue_ packet data available last_rx_ %d now - last_rx_ %d", this->last_rx_, now - this->last_rx_);
	//if (now - this->last_rx_ > 100) {
	//  ESP_LOGI(TAG, "now - last_rx_ > 100 %d", now - this->last_rx_);  
    //  ESP_LOGI(TAG, "now - last_poll_start_ < 9500 %d", now - this->last_poll_start_); 
	//  ESP_LOGI(TAG, "now - last_tx_ > 200 %d", now - this->last_tx_);  	
	//}
  //}


  // My own working values
  // if (now - this->last_rx_ > 100) {	  
  // 	if (now - this->last_poll_start_ < 2000) {	   		
  // 	  if (now - this->last_tx_ > 100) {		
...
bool UponorSmatrixComponent::parse_byte_(uint8_t byte) {
  ...
  //if (device_address == 0x2ccc)
  //{
  //  ESP_LOGI(TAG, "received packet i-143: sys=%04x, dev=%04x, data=%s, crc=%04x", system_address, device_address,
  //		format_hex_pretty(&packet[4], packet_len - 6).c_str(), crc);	  
  //}
  ...
  if (!found && !this->unknown_devices_.count(device_address)) {
    ESP_LOGI(TAG, "Received packet for unknown device systemaddress address 0x%04X 0x%04X ", system_address, device_address);
...
bool UponorSmatrixComponent::send(uint16_t system_address, uint16_t device_address, const UponorSmatrixData *data, size_t data_len) {
  if (this->address_ == 0 || device_address == 0 || data == nullptr || data_len == 0)
    return false;
  
  ESP_LOGI(TAG, "Send data to systemaddress address 0x%04X 0x%04X", system_address, device_address);
  ...
  packet.push_back(system_address >> 8);
  packet.push_back(system_address >> 0);
...
bool UponorSmatrixComponent::do_send_time_() {
  ...
  return this->send(this->address_, this->time_device_address_, data, sizeof(data) / sizeof(data[0]));
}
#endif
