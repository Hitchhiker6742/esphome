...
void UponorSmatrixComponent::dump_config() {
  ...
  //ESP_LOGCONFIG(TAG, "  System address: 0x%08X", this->address_);
#ifdef USE_TIME
  if (this->time_id_ != nullptr) {
...
bool UponorSmatrixComponent::parse_byte_(uint8_t byte) {
  ...
  if (!found && !this->unknown_devices_.count(device_address)) {
    ESP_LOGI(TAG, "Received packet for unknown device systemaddress address 0x%08X 0x%04X ", device_address);
...
bool UponorSmatrixComponent::send(uint32_t device_address, const UponorSmatrixData *data, size_t data_len) {
  ...
  ESP_LOGI(TAG, "Send data to systemaddress address 0x%08X", device_address);
