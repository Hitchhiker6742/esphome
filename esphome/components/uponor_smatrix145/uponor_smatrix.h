...
/// Timer I-143 vacation temperature
static const uint8_t UPONOR_ID_TIMER_VACATION_TEMP = 0x03;
...
/// Timer I-143 Temperature Setpoint Setback for ECO Mode
static const uint8_t UPONOR_ID_TIMER_ECO_SETBACK = 0x0D;
/// Outdoor Temperature? (sent by controller)
static const uint8_t UPONOR_ID_OUTDOOR_TEMP = 0x2D;
/// Timer I-143 heating mode/vacation/autocalibration
static const uint8_t UPONOR_ID_TIMER_HEATING_MODE = 0x32;
...
class UponorSmatrixComponent : public uart::UARTDevice, public Component {
 public:
  ...
  bool send(uint16_t system_address, uint16_t device_address, const UponorSmatrixData *data, size_t data_len);
...
class UponorSmatrixDevice : public Parented<UponorSmatrixComponent> {
 public:
  ...
  void set_system_address(uint16_t address) { this->system_address_ = address; }
  ...
  bool send(const UponorSmatrixData *data, size_t data_len) {
    return this->parent_->send(this->system_address_, this->address_, data, data_len);
  }

 protected:
  friend UponorSmatrixComponent;
  uint16_t address_;
  uint16_t system_address_;  
};

inline float raw_to_celsius2(uint16_t raw) {
  return (raw == UPONOR_INVALID_VALUE) ? NAN : (raw / 1.8f / 10.0f);
}
